...
<div class="accordion" id="menu-${param.parentId}">
	...
	<c:forEach items="${menuList}" var="menu" varStatus="idxStatus">
		<c:if
			...
			<div class="accordion-group" style="margin-bottom:0px;border-radius:0px;border:0px;">
				<div class="accordion-heading" style="border-radius:0px;background-image:none;background:#f2f2f2;">
					<a class="accordion-toggle" style="/* color:#fff !important; */padding:13px 20px;" data-toggle="collapse"
						
						data-href="#collapse-${menu.id}" href="#collapse-${menu.id}"
						title="${menu.remarks}"><i
						class="icon-${not empty menu.icon ? menu.icon : 'circle-arrow-right'}""></i>&nbsp;${menu.name}<i
						class="icon-chevron-${not empty firstMenu && firstMenu ? 'down' : 'right'}" style="margin-left:60px;"></i></a>
				</div>
				<div id="collapse-${menu.id}"
					class="accordion-body collapse ${not empty firstMenu && firstMenu ? 'in' : ''}">
					<div class="accordion-inner" style="border-top:0px solid #e5e5e5;padding:0px 0px 0px 0px;">
						<ul class="nav nav-list" style="margin-bottom:0px;">
							<c:forEach items="${menuList}" var="menu2">
								<c:if test="${menu2.parent.id eq menu.id&&menu2.isShow eq '1'}">
									<li><a data-href=".menu3-${menu2.id}"  id="${menu2.id}" style="margin-top:0px;margin-bottom:0px;text-shadow:none;padding:10px 0px 10px 35px;"
