...
<link rel="stylesheet" type="text/css"
	...
	media="all">
<link rel="stylesheet" type="text/css"
	href="${pageContext.request.contextPath}/static/plugins/layui/css/app.css">
<script
	src="${pageContext.request.contextPath}/static/plugins/layui/layui.js"
...
<script type="text/javascript">
	$(function() {
		//判断若已经接班则不在弹出接班请求
		$.ajax({
			url : '${ctx}/duty/wmOnduty/findWmOndutyByStatus',
			dataType : 'json',
			type : 'post',
			success : function(msg) {
				if (msg.changeStatus == '0') {
					layui.use([ 'laydate', 'laypage', 'layer', 'table', 'carousel', 'upload', 'element' ], function() {
						...
						var index = layui.layer.open({
							...
							type : 2,
							area : [ '1500px', '900px' ],
							content : "${ctx}/duty/wmOnduty/firstJiaoJiebanFrom",
...
<script type="text/javascript">
					...
					$.get($(this).attr("data-href"), function(data){
						...
						$(menuId + " .accordion-heading a").click(function(){
							if(!$($(this).attr('data-href')).hasClass('in')){
								$(this).children("i:last-child").removeClass('icon-chevron-right').addClass('icon-chevron-down');
							}else{
								$(this).children("i:last-child").removeClass('icon-chevron-down').addClass('icon-chevron-right');
							}
						});
						// 二级内容
		...
		//待办事项点击申领（补办）跳转到我的工作界面
		function cursorToUnionSealToSignList(){
				/* if($("#5298467c88d049239828267237da216e").hasClass("icon-chevron-right")){
				$("#5298467c88d049239828267237da216e").click();
			} */
			$("#17b7c64ce04d467387bb8e80db34f8e3").click();
		}
		
		//打开历史消息列表
		function cursorToUnionSealToOpenNews(){
				if($("#7fbf3d9136db42fdb849d27b2a966a6b").hasClass("icon-chevron-right")){
				$("#7fbf3d9136db42fdb849d27b2a966a6b").click();
			}
			$("#9fc1e88f0b0646ec9114cb35c0debd1f").click();
		}
		
		
			function hasMsg(){
				$("#msgImg").attr("src","${ctxStatic}/images/navimg.gif");
				 $("#tochecknews").removeClass("news");
				 $("#tochecknews").addClass("hasmessage");
				
				
			}
			function notHasMsg(){
				$("#msgImg").attr("src","${ctxStatic}/images/messagestatic.png");
			}
			//查询消息接收数
			function notifyBacklog(){
		//alert("yes!");
				var num = 0;
				$.ajax({
					url: "${ctx}/wmnewsreceivers/wmNewsReceiver/tochecknum",
					//async: false,
					type: "POST",
					dataType:'json',
		...
		//	alert("come in ");
					...
					var refresh = window.mainFrame.refresh;
					if(refresh != null && refresh != undefined && typeof(refresh) == 'function'){
						refresh();
					}
				},
				30000);
...
<body>
	<div id="main">
		<div id="header" class="navbar navbar-fixed-top">
			...
			<div style="background:url(${ctxStatic}/images/nav.png)"
				class="navbar-inner">
				<div class="brand">
					<img class="title_img" src="${ctxStatic}/images/nav_logo.png"
						alt="" /> <span id="productName" style="font-size: 32px;">${fns:getConfig('productName')}</span>
				</div>
				<ul id="userControl" class="nav pull-right">
					...
					<li id="userInfo" class="dropdown"><a class="dropdown-toggle"
						data-toggle="dropdown" href="#" title="个人信息"><i
							class="layui-icon" style="padding-right:5px;"> &#xe612;</i>欢迎您！&nbsp;${fns:getUser().name}&nbsp;<span
							id="notifyNum" class="label label-info hide"></span></a>
					<li class="layui-nav-item ">
						<!-- 通知 --> <a  href="javascript:void(0)" onclick="" id="tochecknews" style="color: #fff; text-align: center"
						class="news"> <img id="msgImg" alt=""
							style="width:13%;margin-right: 5px;vertical-align: middle"
							src="${ctxStatic}/images/messagestatic.png" ><span
							id="msg">通知</span></a> <%-- <img  style="width:13%;margin-right: 5px;vertical-align: middle" src="${ctxStatic}/images/navimg.gif" alt=""/> --%>
						</a>
					</li>
					<ul class="dropdown-menu">
					...
					<li><a href="${ctx}/logout" title="退出登录"><i
							class="layui-icon" style="padding-right:5px;">&#xe620;</i>退出</a></li>
					<li>&nbsp;</li>
				</ul>
		...
		<div class="container-fluid">
			...
			<div id="footer" class="row-fluid">Copyright &copy; 2018
				${fns:getConfig('productName')} ${fns:getConfig('version')}</div>
		</div>
	</div>
	<script type="text/javascript">
